...
export default function Login() {
	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			if (res.ok) {
				...
				// Mostrar mensajes claros según la respuesta del backend
	...
	return (
		<div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 via-purple-50 to-pink-50 p-6">
			<div className="w-full max-w-md mx-auto bg-white/60 backdrop-blur-md rounded-2xl shadow-lg p-8 border border-white/30">
				<div className="text-center mb-6">
					<h1 className="text-2xl font-semibold text-purple-700">Bienvenido</h1>
					<p className="text-sm text-purple-600/80">Inicia sesión para continuar</p>
				</div>

				{error && <p className="bg-red-100 text-red-600 p-2 rounded-md text-center mb-4 text-sm">{error}</p>}

				<form className="space-y-4" onSubmit={handleSubmit} aria-label="formulario de inicio de sesión">
					<div>
						<label className="block text-sm font-medium text-gray-700 mb-1">Usuario (name)</label>
						<input
							type="text"
							name="name"
							value={form.name}
				...
				<p className="mt-6 text-center text-sm text-gray-600">¿No tienes cuenta? <Link to="/register" className="text-purple-600 font-medium">Regístrate</Link></p>
			</div>
		</div>
	);
}
